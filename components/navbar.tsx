    ...
    <HeroUINavbar className="bg-[#f3ede7]" maxWidth="xl" position="sticky">
      <NavbarContent className="basis-1/5 sm:basis-full" justify="start">
        <NavbarBrand className="gap-3 max-w-fit">
          <NextLink className="flex justify-start items-center gap-1" href="/">
            <Image src="/rr.png" alt="Logo" />
          </NextLink>
        </NavbarBrand>
      </NavbarContent>

      <NavbarContent className="hidden sm:flex sm:basis-full" justify="end">
        <NavbarItem className="hidden md:flex justify-around w-full gap-3">
          {isAuthenticated ? (
      ...
      <NavbarMenu>
        <div className="mx-4 mt-2 flex flex-col">
          {isAuthenticated ? (
            // Si l'utilisateur est authentifié, afficher ces liens
            <>
              <NavbarMenuItem>
            ...
            // Si l'utilisateur n'est pas authentifié, afficher ces liens
            <>
              <NavbarMenuItem>
